...
// Optional env vars: CONTACT_TO_EMAIL, CONTACT_FROM_EMAIL, CONTACT_REPLY_DOMAIN.
...
module.exports = async (req, res) => {
  ...
  const date        = pickField(body, "date");
  const description = pickField(body, "description", "message");
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "0nleiter@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "AllStar Painting <onboarding@resend.dev>";

  const subject = `New lead from allstarpaintinganddrywall.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on allstarpaintinganddrywall.com`;
  const html = `
<h2>New lead from the AllStar Painting site</h2>
<table cellpadding="6" style="font-family:system-ui,Arial,sans-serif;border-collapse:collapse;font-size:15px">
  ...
  <tr><td valign="top"><b>Job description</b></td><td>${escapeHtml(description).replace(/\n/g, "<br>")}</td></tr>
</table>
<p style="color:#666;font-size:13px">Sent from the contact form on allstarpaintinganddrywall.com</p>
`;
